<?php
	...
	if(!empty($_POST))
	{
		...
		if($_POST['action'] == 'infoProducto')
		{
			...
			$query = mysqli_query($conection,"SELECT p.codproducto,p.producto,p.descripcion,p.precio_compra,p.precio,p.existencia,p.existencia_minima,p.coditem,p.codebar,DATE_FORMAT(p.date_add, '%d/%m/%Y') as fecha,p.foto,mr.idmarca, mr.marca,p.presentacion_id,c.categoria,pr.presentacion
											FROM producto p
											INNER JOIN marca mr
		...
		if($_POST['action'] == 'infoProductoCod')
		{
			...
			$query = mysqli_query($conection,"SELECT p.codproducto,p.producto,p.descripcion,p.precio_compra,p.precio,p.existencia,p.existencia_minima,p.codebar,mr.idmarca, mr.marca
											FROM producto p
											INNER JOIN marca mr
											ON p.marca_id = mr.idmarca
		...
		if($_POST['action'] == 'infoProductoSearch')
		{
			if($_SESSION['active'])
			{
				if(!empty($_POST['searchText']))
				{
					...
					$querySearchPro = mysqli_query($conection,"SELECT p.codproducto, p.producto, p.descripcion,p.precio_compra, p.precio, p.existencia , mr.marca,p.codebar,p.coditem
													FROM producto p
													INNER JOIN marca mr
													ON p.marca_id = mr.idmarca
													WHERE
													p.codebar LIKE '%$busqueda%' OR p.coditem LIKE '%$busqueda%' OR
		...
		// Extrae producto por nombre, codigo item, marca,cateroria - COMPRA
		if($_POST['action'] == 'infoProductoSearch_v')
		{
			if($_SESSION['active'])
			{
				if(!empty($_POST['searchText']))
				{
					...
					$querySearchPro = mysqli_query($conection,"SELECT p.coditem,p.codproducto,p.producto,p.descripcion,p.precio_compra, p.precio, p.existencia , mr.marca,p.codebar
													FROM producto p
													INNER JOIN marca mr
													ON p.marca_id = mr.idmarca
													WHERE
		...
		*/
		...
		if($_POST['action'] == 'addProductoDetalle'){
			...
			if(empty($_POST['producto']) || empty($_POST['cantidad']))
			{
				...
				$queryProducto = mysqli_query($conection,"SELECT precio_compra, precio,existencia,impuesto_id FROM producto WHERE codproducto = $codproducto;");
