...
<?php
	...
	{
		$alert='';
		if(  empty($_POST['codBarra']) || empty($_POST['codItem']) || empty($_POST['producto'])  || empty($_POST['descripcion']) || empty($_POST['marca']) || empty($_POST['categoria']) || empty($_POST['presentacion']) || empty($_POST['cantMinima'])|| empty($_POST['existencia'])|| empty($_POST['precioCompra']) || empty($_POST['precio']) || empty($_POST['id_impuesto']))
		{
			$alert='<p class="msg_error">Todos los campos son obligatorios.</p>';
		}else{
			$codproducto = ucfirst($_POST['id']);
			$barcode   	 = $_POST['codBarra'];
			$itemCode	 = $_POST['codItem'];
			...
			if(empty($_POST['codBarra']) || empty($_POST['codItem']))
			{
				$fecha 			= date('ymd');
				$hora 			= date('Hms');
				$barcode = $fecha.$hora;
			...
			$query = mysqli_query($conection,"SELECT * FROM producto WHERE   coditem = '$itemCode' AND codproducto != $codproducto");
			$result = mysqli_num_rows($query);
			
			if($result > 0){
				$alert='<p class="msg_error">El código de barra ya existe o El Codigo Item se Repite.</p>';
			}else{
				  
				$query_update = mysqli_query($conection,"UPDATE producto
																SET coditem = '$itemCode',
																	producto = '$producto',
	...
	if(empty($_REQUEST['id']) )
	{
		...
		$query = mysqli_query($conection,"SELECT p.codproducto,p.producto,p.descripcion,p.precio_compra
		,p.precio,p.existencia,p.existencia_minima,p.codebar,p.coditem,p.ubicacion_id,p.presentacion_id,p.foto,p.impuesto_id,i.descripcion as desc_impuesto,mr.idmarca,c.idcategoria,c.categoria, mr.marca,pr.presentacion,u.ubicacion
											FROM producto p
											INNER JOIN marca mr
		...
		<div class="form_producto">
			...
			<form class="form" action="" method="post" enctype="multipart/form-data">
				...
				<div class="form_container wd50">
					...
					<label for="codItem">Código Item</label>
						<input type="text" name="codItem" id="codItem" placeholder="Código Item" value="<?= $data['coditem']; ?>">
					</div>
					...
					<div class="wd30">
						<label for="precio">Precio Compra actual</label>
						<input type="text" name="precioCompra" id="precioCompra" value="<?php echo $data['precio_compra']; ?>" placeholder="Precio del producto">
					...
					<div class="wd30">
						...
						<input type="text" name="existencia" id="existencia" value="<?php echo $data['existencia']; ?>" placeholder="cuanto en base de datos">
					</div>
					<div class="wd50">
