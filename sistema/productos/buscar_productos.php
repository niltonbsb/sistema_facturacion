<?php
	...
	if(!empty($_REQUEST['busqueda'])){
		$busqueda = strtolower($_REQUEST['busqueda']);
		$where ="(p.codebar LIKE '%$busqueda%' OR p.coditem LIKE '%$busqueda%' OR p.producto LIKE '%$busqueda%' OR p.descripcion LIKE '%$busqueda%') AND p.estatus = 1";
	...
	<div class="containerTable">
		<table id="tblProductos">
			<tr>
				...
				<th>Categoría</th>
				<th>Marca</th>
				<th class="textright" width="128px">Precio Compra</th>
				<th class="textright" width="128px">Precio Venta</th>
		...
		<?php
			...
			?>
				<tr class="row<?php echo $data["codproducto"]; ?>" id="row_<?php echo $data["codproducto"]; ?>">
					...
					<td class="celPrecio textright"><?php echo SIMBOLO_MONEDA.'. '.formatCant($data["precio_compra"]); ?></td>
