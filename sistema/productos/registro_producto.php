...
<?php
	...
	if(!empty($_POST))
	{
		...
		if( empty($_POST['codItem']) || empty($_POST['producto'])  || empty($_POST['descripcion']) || empty($_POST['marca']) || empty($_POST['categoria']) || empty($_POST['presentacion']) || empty($_POST['cantMinima']) || empty($_POST['precio']) || empty($_POST['id_ubicacion']) || empty($_POST['id_impuesto']))
		{
			...
			$query = mysqli_query($conection,"SELECT * FROM producto WHERE codebar = '$barcode' OR coditem = '$itemCode'");
			$result = mysqli_num_rows($query);

			if($result > 0){
				$alert='<p class="msg_error">El código de barra ya existe o el código item se repite.</p>';
			}else{
				$query_insert = mysqli_query($conection,"INSERT INTO producto(coditem,
																				producto,
																				descripcion,
																				categoria,
																				marca_id,
		...
		<div class="form_producto">
			...
			<form class="form" action="" method="post" enctype="multipart/form-data">
				<div class="form_container wd50">
					...
					<div class="wd50">
						<label for="codBarra">Código Item</label>
						<input type="text" name="codItem" id="codItem" placeholder="Código de barra" value="<?= $itemCode; ?>">
					</div>
					<div class="wd50">
					...
					<div class="wd30">
						...
						<input type="text" name="precioCompra" id="precioCompra" value="" placeholder="Precio Compra" value="<?= $precioCompra; ?>" required >
